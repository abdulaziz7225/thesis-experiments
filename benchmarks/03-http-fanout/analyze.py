"""
Analyse k6 + cold/warm-start + Prometheus results for 03-http-fanout (I/O-bound).
Generates individual chart PNGs into results/03-http-fanout/<mode>/charts/.

Workload class: I/O-bound. The /fanout endpoint dispatches N concurrent
outbound HTTP calls to an in-cluster echo backend that sleeps for delay_ms,
so throughput is gated by I/O wait rather than CPU work.

Expected input files (produced by run_experiment.sh):
  results/03-http-fanout/<mode>/<variant>_summary.json
  results/03-http-fanout/<mode>/<variant>_k6.json       (optional time-series)
  results/03-http-fanout/cold_start.json
  results/03-http-fanout/warm_start.json
  results/03-http-fanout/resource_metrics.json
  results/03-http-fanout/image_sizes.json
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Scaling mode to analyse (reads from results/03-http-fanout/<mode>/)",
    )
