...
from shared.utils import VARIANT_COLORS, VARIANT_LABELS, VARIANTS, results_path
...
def _bar(ax: plt.Axes, values: list[float | None], ylabel: str, title: str,
         fmt: str = "{:.0f}") -> None:
...
def plot_latency(ax: plt.Axes, summaries: dict[str, dict | None]) -> None:
...
def plot_throughput(ax: plt.Axes, summaries: dict[str, dict | None]) -> None:
    vals = [_k6_metric_val(summaries.get(v), "http_reqs", "rate") for v in ORDERED_VARIANTS]
...
def plot_rps_over_time(ax: plt.Axes,
...
def _startup_panel(ax: plt.Axes, data: list[dict], title: str, ylabel: str) -> None:
    ...
    sorted_data = sorted(
        [e for e in data if e["variant"] in ORDERED_VARIANTS and e.get("stats")],
...
def plot_cold_start(ax: plt.Axes, cold_data: list[dict]) -> None:
    _startup_panel(ax, cold_data,
                   "Cold-start latency (mean ± stdev)",
                   "Time to first response (ms)")


def plot_warm_start(ax: plt.Axes, warm_data: list[dict]) -> None:
    _startup_panel(ax, warm_data,
                   "Warm-start latency (mean ± stdev)",
                   "Time to first response (ms)")


def plot_memory(ax: plt.Axes, resource: dict[str, dict]) -> None:
    """Grouped bar: idle vs peak memory (MB) per variant."""
    x = np.arange(len(ORDERED_VARIANTS))
    width = 0.35

    idle_vals = [resource.get(v, {}).get("memory_idle_mb") for v in ORDERED_VARIANTS]
    peak_vals = [resource.get(v, {}).get("memory_peak_mb") for v in ORDERED_VARIANTS]
...
def plot_cpu(ax: plt.Axes, resource: dict[str, dict]) -> None:
    """Average CPU millicores during load test."""
    vals = [resource.get(v, {}).get("cpu_avg_mcores") for v in ORDERED_VARIANTS]
    _bar(ax, vals, "CPU (millicores)", "Avg CPU during load test", fmt="{:.0f}")


def plot_image_sizes(ax: plt.Axes, sizes: dict[str, float]) -> None:
